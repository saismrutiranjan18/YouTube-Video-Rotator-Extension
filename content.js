// ============================================================
// YouTube Rotate Pro — content.js  v1.2.0
...
(function () {
  ...
  function buildPanel() {
    ...
    panel.innerHTML = `
      ...
      <div id="yrp-badge">0°</div>
    `;
    document.body.appendChild(panel);

    // Restore saved position or panelHidden
    chrome.storage.local.get(['panelX', 'panelY', 'panelHidden', 'controlPosition'], (r) => {
      if (r.panelX !== undefined && r.panelY !== undefined) {
        panel.style.top   = r.panelY + 'px';
        panel.style.left  = r.panelX + 'px';
        panel.style.right = 'auto';
      } else if (r.controlPosition === 'Bottom Right') {
  ...
  function attachEvents() {
    ...
    document.addEventListener('mouseup', () => {
      ...
      chrome.storage.local.set({
        panelX: parseInt(panel.style.left)  || 0,
        panelY: parseInt(panel.style.top)   || 0
      });
    });
  }
  ...
  function applyTransform() {
    ...
    if (badge) {
      const hasChange = rotation !== 0 || flipH || flipV;
      badge.style.display = hasChange ? 'block' : 'none';
      let label = rotation + '°';
      if (flipH && flipV) label += ' ⇔⇕';
      else if (flipH) label += ' ⇔';
      else if (flipV) label += ' ⇕';
  ...
  document.addEventListener('keydown', (e) => {
    ...
    if (e.altKey && e.shiftKey && e.key.toLowerCase() === 'r') {
      ...
      if (document.getElementById('yrp-fh')) document.getElementById('yrp-fh').classList.remove('yrp-active');
      if (document.getElementById('yrp-fv')) document.getElementById('yrp-fv').classList.remove('yrp-active');
      applyTransform(); save();
    }
    ...
    if (e.altKey && !e.shiftKey && e.key.toLowerCase() === 'h') {
      e.preventDefault();
      flipH = !flipH;
      if (document.getElementById('yrp-fh')) document.getElementById('yrp-fh').classList.toggle('yrp-active', flipH);
      applyTransform(); save();
    }
    // ALT + ArrowUp → +1°
